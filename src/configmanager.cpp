...
float_t getGlobalFloat(lua_State* L, const char* identifier, const float_t defaultValue = 0.0)
{
	lua_getglobal(L, identifier);
	if (!lua_isnumber(L, -1)) {
		return defaultValue;
...
float_t ConfigManager::getFloat(floating_config_t what) const
{
